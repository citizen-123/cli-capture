...
func renderFlowDetail(f *capture.Flow, width int, expand bool) string {
	...
	fmt.Fprintf(&b, "%s  %s\n", sanitizeCaptureText(string(f.Protocol)), sanitizeCaptureText(f.Title()))
	fmt.Fprintf(&b, "server %s   sni %s   secure %v   status %s\n",
		sanitizeCaptureText(f.ServerAddr), dash(sanitizeCaptureText(f.SNI)), f.Secure, sanitizeCaptureText(f.Status.String()))
	if f.Err != nil {
		fmt.Fprintf(&b, "error: %s\n", sanitizeCaptureText(f.Err.Error()))
	}
	...
	if len(f.Messages) > 0 {
		...
		for i, m := range f.Messages {
			...
			fmt.Fprintf(&b, "%s %s\n", m.Direction, sanitizeCaptureText(m.Summary))
			if len(m.Body) > 0 {
...
func renderMessage(m *capture.Message, width int, expand bool) string {
	...
	for _, k := range sortedKeys(m.Headers) {
		for _, v := range m.Headers[k] {
			fmt.Fprintf(&b, "%s: %s\n", sanitizeCaptureText(k), sanitizeCaptureText(v))
		}
	}
	if len(m.Meta) > 0 {
		for _, k := range sortedKeys2(m.Meta) {
			...
			fmt.Fprintf(&b, "· %s: %s\n", sanitizeCaptureText(k), sanitizeCaptureText(m.Meta[k]))
		}
	}
	if len(m.Body) > 0 {
...
func bodyPreview(body []byte, expand bool) string {
	...
	if isMostlyPrintable(b) {
		s = sanitizeCaptureText(string(b))
	} else {
		s = hexDump(b)
	}
	if truncated {
...
func colorizeJSON(s []byte, expand bool, embedDepth int) string {
	...
	for i := 0; i < len(s); {
		...
		case c == '"':
			...
			str := string(s[i:j])
			safeStr := sanitizeCaptureText(str)
			k := j
			for k < len(s) && (s[k] == ' ' || s[k] == '\t') {
				k++
			}
...
func expandString(raw, prefix string, embedDepth int) (string, bool) {
	...
	if strings.Contains(decoded, "\n") {
		return markExpanded("↳ expanded (escaped \\n)", jsonStrStyle.Render(sanitizeCaptureText(decoded)), prefix), true
	}

	return "", false
}

// sanitizeCaptureText renders every untrusted C0, DEL, and C1 control as a
// visible escape before it reaches a terminal. It deliberately escapes line
// breaks too: only layout generated by a renderer may control its output.
func sanitizeCaptureText(s string) string {
	return sanitizeCaptureControls(s, false)
}

// sanitizeCaptureMultilineText applies the same terminal-control policy to an
// editable capture while retaining generated line breaks for textarea layout.
// CRLF is normalized to LF first; a bare CR remains visibly escaped rather
// than being allowed to move the terminal cursor.
func sanitizeCaptureMultilineText(s string) string {
	return sanitizeCaptureControls(strings.ReplaceAll(s, "\r\n", "\n"), true)
}

func sanitizeCaptureControls(s string, keepNewlines bool) string {
	var b strings.Builder
	for _, r := range s {
		switch {
		case r == '\n' && keepNewlines:
