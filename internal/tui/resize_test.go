...
// overflow the terminal. Each content width maps to a lipgloss box of width+2
// (a border column on each side), and the two boxes must sum to exactly
// m.width — so the two content widths sum to m.width - 4 — for every ratio.
func TestPaneWidthsFitAndRespectRatio(t *testing.T) {
	...
	for _, r := range []float64{minSplit, 0.35, 0.5, 0.65, maxSplit} {
		...
		if l+right != w-4 {
			t.Errorf("ratio %v: left(%d)+right(%d) = %d, want %d", r, l, right, l+right, w-4)
		}
		// The rendered boxes (content + 2 border columns each) must fit exactly.
		if (l+2)+(right+2) != w {
			t.Errorf("ratio %v: rendered boxes %d+%d != terminal width %d", r, l+2, right+2, w)
		}
		if l < 1 || right < 1 {
	...
	// A 50/50 split gives each pane an equal box, so the left content width is
	// w/2 - 2 (the box is w/2, less its two border columns).
	if got := (Model{width: w, splitRatio: 0.5}).leftPaneWidth(); got != w/2-2 {
		t.Errorf("leftPaneWidth at 0.5 = %d, want %d", got, w/2-2)
	}
